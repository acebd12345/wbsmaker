...
- **LLM 預設用 mock**:真端點設定放 `wbs.local.toml`(gitignored,config 合併順序 defaults ← wbs.toml ← wbs.local.toml);repo 內的 wbs.toml 只放可共享預設值,不得寫入內網主機名。`llm/client.py` 支援 `mock: true`(依 schema 生成假資料),s09–s13 的開發與測試全部在 mock 下完成。Stage s01–s08 完全不依賴 LLM。
...
- **規則外部化(P1 之後)**:s04/s06/s09 的切分/章節/分類規則全部住在 `profiles/default.toml`,程式碼禁止出現業務字面量(有靜態檢查測試擋)。改行為 = 改 profile,金絲雀/反金絲雀測試會驗證 profile 真的在驅動 stage。案例庫在 `cases/`(指紋+答案,無契約原文),`wbs eval run` 出分層記分卡,合併 profile 前 merge_gate 必須 allow。
- Commit 習慣:每完成任務書一步就 commit,訊息格式「<步驟代號>: <內容>」。遠端 origin=GitHub 私有 repo;`master`=一期穩定版,`learning-system`=學習系統支線。data/、reports/、wbs.local.toml、cases/locators.local.toml 不進 git。
